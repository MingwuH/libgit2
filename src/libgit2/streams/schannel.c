...
static ssize_t schannel_read(git_stream *stream, void *_data, size_t data_len)
{
    ...
    /*
     * Loop until we have some bytes to return - we may have decrypted bytes queued
     * or ciphertext from the wire that we can decrypt and return. Return any queued
     * bytes if they're available to avoid a network read, which may block. We may
     * return less than the caller requested, and they can retry for an actual network
     */
    while ((size_t)total_len < data_len) {
	printf("read loop: %" PRIuZ " / %" PRIuZ "\n", total_len, data_len);
